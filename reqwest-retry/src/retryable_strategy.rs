...
/// A strategy to create a [`Retryable`] from a [`Result<wreq::Response, reqwest_middleware::Error>`]
///
/// A [`RetryableStrategy`] has a single `handler` functions.
/// The result of calling the request could be:
/// - [`wreq::Response`] In case the request has been sent and received correctly
...
///     fn handle(&self, res: &Result<wreq::Response>) -> Option<Retryable> {
///          match res {
///              // retry if 201
...
pub trait RetryableStrategy {
    fn handle(&self, res: &Result<wreq::Response, Error>) -> Option<Retryable>;
}
...
pub fn default_on_request_success(success: &wreq::Response) -> Option<Retryable> {
    ...
    match error {
        ...
        Error::Rquest(error) => {
                ...
                || error.is_redirect()
            {
                ...
                // It seems that hyper::Error(IncompleteMessage) is not correctly handled by wreq.
                // Here we check if the wreq error was originated by hyper and map it consistently.
                #[cfg(not(target_arch = "wasm32"))]
